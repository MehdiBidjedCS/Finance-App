...
import Account from "./Accountchois.jsx";
function Signup() {
  const [currentPage, setCurrentPage] = useState('sign-up');
  {/* temporel function */}
  const goToAddAccount=()=>setCurrentPage('add-account');
        ...
        <div className="w-full h-full rounded-b-[20px] flex justify-around items-center">
          <div className="mt-[60px] ml-[50px] w-[60%] h-full flex flex-col items-start gap-[10px]">
            ...
            <div className="flex flex-col items-start gap-[10px]">
              <p>Email :</p>
              <input className="outline-none border border-[#c9c9c9] p-[5px] rounded-[4px]" type="text" placeholder="Emai" required />
              <p>Password :</p>
              <input className="outline-none border border-[#c9c9c9] p-[5px] rounded-[4px]" type="text" placeholder="Password" required />
              <div className="flex justify-between gap-[10px] p-[5px]">
                <input className="outline-none border border-[#c9c9c9]  rounded-[4px]" type="checkbox" required />
                <p>
                  by continuing , I agree to the terms of use & privacy policy.
                </p>
              </div>
              <button className="border-none w-[100px] p-[10px] text-center rounded-[4px] text-white bg-purple-600 text-[15px] cursor-pointer" onClick={goToAddAccount}>Sign Up</button>
            </div>
          </div>
          <div className="h-full">
            <img className="w-full h-[80%] rounded-b-[20px]" src={assets.test} alt="" />
          </div>
        </div>
      </form>
      }
      {
        currentPage === 'add-account'&& <Account currentPage={currentPage} setCurrentPage={setCurrentPage}/>
      }
